...
public class ControllerHindImageReco2 {
    @FXML
    ...
    public static void HindImageReco2(FlowPane flowPaneHFYuan) {
        ...
        for (int theNumberPicture = 0; theNumberPicture < ivsZT.size(); theNumberPicture++) { //按顺序获得用户选择的图像。
            ...
            int t = (widthCarrier * heightCarrier * a) / ((recoverNecessaryInf[0] * recoverNecessaryInf[1]) * 8 + 20); // 隐藏bit值每一组的个数。
            ...
            while (flag) {
                for (int intColorCarrier = 16; intColorCarrier >= 0; intColorCarrier -= 8) {
                    ...
                    int m1 = x1 ^ x2;
                    rgbHind[rgbNum] = (rgbHind[rgbNum] << 1) | m0; // 累计一个隐藏图像的颜色像素值。
                    rgbHind[rgbNum] = (rgbHind[rgbNum] << 1) | m1; // 累计一个隐藏图像的颜色像素值。
                    num += 2; // 计数从载体图像中出去的bit值的个数。
                    if (num % 6 == 0) {
                        l++;
                    ...
                    if (jHind == recoverNecessaryInf[1]) {
                        flag = false;
                        break;
                    }
                }
