...
public class ControllerHindImageAKSGS20192 {
    @FXML
    ...
    void TrueHind(ActionEvent event) {
        ...
        for (int theNumberPicture = 0; theNumberPicture < imageHind.length; theNumberPicture++) { //按顺序获得用户选择的图像。
            ...
            while (flag) {
                ...
                int pxnew = ControllerHindImageAKSGS2019.pxpx1New1(dec1, rem1, x1, x2, n, px);
                int pxnew1 = ControllerHindImageAKSGS2019.pxpx1New1(dec2, rem2, y1, y2, n, px1);
                ...
                if (nnew == 3) {
                    pixelGroup = obtainLastPixel11(nnew, pxnew, pxnew1);
                } else if (nnew == 4) {
                    pixelGroup = obtainLastPixel12(nnew, pxnew, pxnew1);
                }

                //步骤七 ：如果出现溢出则执行公式。
                if (pixelGroup[0] < 0 | pixelGroup[0] > 255 | pixelGroup[1] < 0 | pixelGroup[1] > 255) {
                    pixelGroup = ControllerHindImageAKSGS2019.obtainLastPixelDisposeFOBP(nnew, pixelGroup);
                }
    ...
    public static int TableOfDifferenceRanges(int difference) {
        ...
        if (difference >= 0 & difference <= 15) {
            n = 3;
        } else if (difference >= 16 & difference <= 255) {
            n = 4;
        }
        return n;
    }

    public static int[] obtainLastPixel11(int n, int pxnew, int pxnew1) {
        int[] pixelGroup = new int[2];
        if (n == 3) {
            pixelGroup[0] = pxnew;
            pixelGroup[1] = pxnew1;
        } else if (n == 4 & pxnew >= pxnew1) {
    ...
    public static int[] obtainLastPixel12(int n, int pxnew, int pxnew1) {
        int[] pixelGroup = new int[2];
        if (n == 4) {
            pixelGroup[0] = pxnew;
            pixelGroup[1] = pxnew1;
