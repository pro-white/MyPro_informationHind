...
public class ControllerHindImagePMTA2018 {
    @FXML
    ...
    void TrueHind(ActionEvent event) {
        ...
        for (int theNumberPicture = 0; theNumberPicture < imageHind.length; theNumberPicture++) { //按顺序获得用户选择的图像。
            ...
            //步骤一 ：循环遍历图像的像素值，将图像分成1*2的非重叠块。
            boolean flag = true;
            while (flag) {

                int z = (pixelReaderCarrier.getArgb(carrierHomej, carrierHomei)) & 0xff; // 将获得rgb值转为十进制。
                carrierHomej++;
                ...
                if (d > 2 || z > 253 || z < 2) {
                    passwordInf.add(1);
                    pw[theNumberPicture].setColor(carrierEndj, carrierEndi, Color.grayRgb(z));
                    carrierEndj++;
                    ...
                    pw[theNumberPicture].setColor(carrierEndj, carrierEndi, Color.grayRgb(y));
                    carrierEndj++;
                    if (carrierEndj == widthCarrier) {
                        carrierEndj = 0;
                        carrierEndi++;
                ...
                int b = ((pixelReaderHind.getArgb(hindHomej, hindHomei) & 0xff) >>> bitNumber--) & 1; // 获得字节中具体下标的二进制值。

                //步骤六 ：将带嵌入信息b与差值d进行相加得到新的差值d’
                int dd = d + b;

                //步骤七 ：将新的差值d‘ 与 原像素值z进行相加得到嵌入信息之后的z’
                int zz = dd + z;

                //步骤九 ：将得到的修改量使用平均法分别写入两幅载体图像中。
                pw[theNumberPicture].setColor(carrierEndj, carrierEndi, Color.grayRgb(zz));
                carrierEndj++;
                if (carrierEndj == widthCarrier) {
                    carrierEndj = 0;
                    carrierEndi++;
...
//            HindInf(widthHind, heightHind, widthCarrier, heightCarrier, pixelReaderCarrier, pw);
            ...
            for (
                    int theNumRecoverNecInf = 0;
                    theNumRecoverNecInf < 5; theNumRecoverNecInf++) {//循环遍历列表recoverNecessaryInf中的元素。
    ...
    void HFHind(ActionEvent event) {
        ...
        for (int theNumberPicture = 0; theNumberPicture < ivsOK.size(); theNumberPicture++) { //按顺序获得用户选择的图像。
            ...
            PixelReader pixelReaderCarrier1 = ivsOK.get(theNumberPicture).getImage().getPixelReader(); //读取第numImage幅载体图像的像素值。
            ...
            while (flag) {
                ...
                int z = (pixelReaderCarrier1.getArgb(carrierHomej, carrierHomei)) & 0xff;
                carrierHomej++;
                if (carrierHomej == widthCarrier) {
                    carrierHomei++;
                    carrierHomej = 0;
                }
                int y = (pixelReaderCarrier1.getArgb(carrierHomej, carrierHomei)) & 0xff;
                carrierHomej++;
                if (carrierHomej == widthCarrier) {
                    carrierHomei++;
                    carrierHomej = 0;
                ...
                //步骤三 ：获得TRT的值，并且进行判断大小：1还是0
                if (passwordInf.get(number) == 1) {
                    number++;
                    continue;
                }
                number++;
                ...
                //步骤四 ：获得嵌入信息b的值
                int b = Math.floorMod(d, 2);
                pixelValues += b * Math.pow(2, bitNumber--); // 从小到大的计算2的幂次。
                if (bitNumber < 0) { // 如果嵌入的数据bit位超过 8位，那么需要换下一个隐藏像素值。
                    bitNumber = 7;
